...
for index, df in exl.iterrows():
    ...
    if is_chargeable and (isinstance(is_chargeable, str) and is_chargeable.lower() in ['t', 'true', 'y', 'yes']):
        is_chargeable = True
    elif is_chargeable and (isinstance(is_chargeable, str) and is_chargeable.lower() in ['f', 'false', 'n', 'no']):
        is_chargeable = False
    else:
        print('Unknown Is-Chargeable value')
        continue

    if commission_type and (isinstance(commission_type, str) and commission_type.lower() in ['p']):
        commission_type = 'P'
    elif commission_type and (isinstance(commission_type, str) and commission_type.lower() in ['f']):
        commission_type = 'F'
    else:
